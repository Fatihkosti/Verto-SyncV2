...
begin
    ...
    if v_profile.is_active is not true then return 'PROFILE_INACTIVE'; end if;
    ...
    if not found then return 'MEMBERSHIP_INVALID'; end if;
...
revoke all on function public.verto_auth_session_status() from public;
revoke all on function public.verto_auth_session_status() from anon;
grant execute on function public.verto_auth_session_status() to authenticated;
grant execute on function public.verto_auth_session_status() to service_role;
